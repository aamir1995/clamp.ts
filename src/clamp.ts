 ...
 */
const computeStyle = (elem: HTMLElement, prop: string): string => {
 ...
 * @returns max lines
 */
const getMaxLines = (element: HTMLElement, height?: number): number => {
    const availHeight = height || element.clientHeight, lineHeight = getLineHeight(element);
 ...
 * @returns max height
 */
const getMaxHeight = (element: HTMLElement, clmp: number): number => {
 ...
 */
const getLineHeight = (elem: HTMLElement): number => {
 ...
 */
const getLastChild = (elem: HTMLElement, options: Options): HTMLElement => {
    //Current element has children, need to go deeper and get last child as a text node
    if ((elem.lastChild as any).children && (elem.lastChild as any).children.length > 0) {
 ...
 * @param options config option
 */
const applyEllipsis = (elem: HTMLElement, str: string, options: Options): void => {
...
const truncate = (
    ...
    if (truncationHTMLContainer) {
        ...
        element.innerHTML = target.nodeValue + ' ' + truncationHTMLContainer.innerHTML + options.truncationChar;
    }
 ...
 */
export function clamp(element: HTMLElement, options?: Options) {
    ...
    const isCSSValue = (clampValue as string).indexOf && ((clampValue as string).indexOf('px') > -1 || (clampValue as string).indexOf('em') > -1);
